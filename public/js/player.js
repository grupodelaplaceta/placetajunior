...
function datosClasificar(d){d=(d&&d.contenido&&typeof d.contenido==='object'&&!Array.isArray(d.contenido))?{...d,...d.contenido}:d||{};const mapa=d.respuestas&&typeof d.respuestas==='object'&&!Array.isArray(d.respuestas)?d.respuestas:{};const raw=Array.isArray(d.elementos)?d.elementos:(Array.isArray(d.palabras)?d.palabras:(Array.isArray(d.items)?d.items:Object.keys(mapa).map(texto=>({texto,categoria:mapa[texto]}))));const items=raw.map(x=>{if(typeof x==='string'){const p=x.split('|');return {texto:p[0]||'',categoria:p[1]||p[2]||mapa[p[0]]||''};}const texto=x.texto??x.palabra??x.nombre??x.enunciado??'';return {texto,categoria:x.categoria??x.categoriaCorrecta??x.categoria_correcta??x.respuesta??x.correcta??x.tipo??mapa[texto]??''};}).filter(x=>String(x.texto).trim()&&String(x.categoria).trim());const cats=(Array.isArray(d.categorias)?d.categorias:(Array.isArray(d.categorias_palabras)?d.categorias_palabras:[])).map(x=>typeof x==='string'?x:(x.nombre??x.titulo??x.texto??x.id??'')).filter(Boolean);return {items,cats:[...new Set(cats.length?cats:items.map(x=>x.categoria).filter(Boolean))]};}
